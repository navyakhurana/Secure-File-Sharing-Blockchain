...
const getFile = async (hash) => {
    ...
    var data = [];
    try{
        const response = await hsContract.methods.get(web3Utils.stringToBytes(hash)).call({ gas: 1000000 });
...
const addFile = async (ipfsHash, fileHash, dateAdded,recipient) => {
    console.log("inside addFile",ipfsHash,fileHash,dateAdded,recipient);
    try{
